...
clients = {}  # Menyimpan koneksi client {username: conn}
...
class Server:
    ...
    def handle_client(self, conn, addr):
        ...
        # Buat folder untuk client jika belum ada
        client_folder = os.path.join(BASE_DIR, f"files_{username}")
        ...
        while True:
            try:
                ...
                if data.startswith("/download"):
                    ...
                    self.send_file(conn, filename, client_folder) 

                elif data.startswith("/send_file"):
                    _, recipient, filename = data.split()
                    self.receive_and_forward_file(conn, recipient, filename)  
                else:
    ...
    def send_file(self, conn, filename, client_folder):
    ...
    def receive_and_forward_file(self, conn, recipient, filename):
        if recipient not in clients:
            conn.sendall(f"Pengguna {recipient} tidak ditemukan atau sedang offline.".encode())
            return
        ...
        # Simpan file di folder penerima
        recipient_folder = os.path.join(BASE_DIR, f"files_{recipient}")
        with open(os.path.join(recipient_folder, file_name), "wb") as f: 
        ...
        print(f"File {filename} berhasil dikirim dari {username} ke {recipient}")
